window.randInRange = function( min, max ){
    return Math.floor(Math.random() * (max - min + 1)) + min;
}

window.checkIntersection = function( a, b ){		
	for( var i = 0; i < a.points.length; i += 2 ){
		if( b.contains( a.points[i], a.points[i+1]) ){
			return true;
		}
	}
	return false;
...
PIXI.gradientTexture = function( width, height, colors ){
	...
	// var tilingSprite = new PIXI.extras.TilingSprite( texture, window.innerWidth, groundHeight * 1.02);
...
window.extend = function( destination, props ){
      if( props === undefined ){
          props = destination;
